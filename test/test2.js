...
describe("EncryptedIQCalculator", function () {
  ...
  function bigIntToHex(bigIntValue) {
    // Remove '0x' prefix if present and pad to even length
  ...
  describe("Group Finalization", function () {
    ...
    it("Should allow owner to finalize group", async function () {
      ...
      h = await iqCalculator.finalizeGroup(groupId, publicKeyForContract);
      console.log(h);
    });
    ...
    it("Should correctly calculate the average", async function () {
      const publicKeyForContract = {
        n: bigIntToHex(publicKey.n),
        g: bigIntToHex(publicKey.g)
      };
      ...
      h = await iqCalculator.finalizeGroup(groupId, publicKeyForContract);
      console.log(h)
      const encryptedAverage = await iqCalculator.getAverage(groupId);
      console.log(encryptedAverage)

      // Convert the encrypted average to BigInt and decrypt
      const encryptedBigInt = BigInt(encryptedAverage);
      const decryptedAverage = Number(privateKey.decrypt(encryptedBigInt));

      // The average of [95, 105, 100] should be 100
      expect(decryptedAverage).to.be.approximately(100, 1);
    });
  });
  ...
  //   it("Should not allow accessing average of non-finalized group", async function () {
  //     await iqCalculator.createGroup(groupId);
  //     await expect(iqCalculator.getAverage(groupId))
